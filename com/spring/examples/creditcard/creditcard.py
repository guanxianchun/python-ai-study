...
def train_predict(best_c_param, x_train, y_train, x_predict, y_test, threshold=0.6):
    ...
    print("*"*80)
    print(u"召回率:反映被正确判断为正例占总的正例的比重,公式: TP/(TP+FN)")
    recall_rate = conf_matrix[1, 1] * 1. / (conf_matrix[1, 1] + conf_matrix[1, 0])
    print(u"测试样本中召回率 R : %s" % (recall_rate))
    print("-"*80)
    print(u"精确度: 反映被分类器判定的正例中,真正的正例所占比重,公式: TP/(TP+FP)")
    pression = conf_matrix[1, 1] * 1. / (conf_matrix[1, 1] + conf_matrix[0, 1])
    print(u"精确度 P : %s" % (pression))
    print("-"*80)
    print(u"准确度: 反映了分类器正确分类的样本数与总样本数之比,公式: (TN+TP)/(TP+FP+FN+TN)")
    accuracy = (conf_matrix[0, 0] +conf_matrix[1,1])* 1. / np.sum(conf_matrix)
    print(u"准确度 A : %s" % (accuracy))
    print("-"*80)
    print(u"F1 Score 2*P*R/(P+R) : %s" % (2 * pression * recall_rate / (pression + recall_rate)))
    print("-"*80)
    print(u"转移性: 负例判断为负例占总的负例样本的比重, 公式: TN/(TN+FP)")
    print(u"转移性S: %s" % (conf_matrix[0, 0] * 1. / (conf_matrix[0, 0] + conf_matrix[0, 1])))
